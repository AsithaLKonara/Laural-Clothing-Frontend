...
export default function ClientContent() {
  ...
  const { data: orderData, isLoading, isError } = useTrackOrderByPhone(searchPhone);
  ...
  // status enum: PENDING, PAID, PROCESSING, DISPATCHED, DELIVERED, CANCELLED
  // fardar status: PENDING, PICKED_UP, IN_TRANSIT, OUT_FOR_DELIVERY, DELIVERED, RETURNED
  
  let currentStep = 0; // Default
  
  if (orderData) {
    if (orderData.status === 'PENDING' || orderData.status === 'PAID') currentStep = 1;
    if (orderData.status === 'PROCESSING') currentStep = 2;
    if (orderData.status === 'DISPATCHED') currentStep = 3;
    if (orderData.status === 'DELIVERED') currentStep = 4;

    if (orderData.trackingInfo) {
      const ts = orderData.trackingInfo.status;
    ...
    <main className="flex flex-col items-center w-full min-h-screen bg-background pt-[83px]">
      ...
      {searchPhone && (
        <section className="flex flex-col lg:flex-row items-start w-full max-w-[1040px] px-4 md:px-[120px] lg:px-0 gap-16 lg:gap-[80px] pb-[100px] animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          {isError ? (
            <div className="w-full p-6 bg-red-50 text-red-600 rounded-lg border border-red-200 text-center">
            ...
            <>
              ...
              <div className="flex flex-col flex-1 w-full gap-[40px]">
                <h2 className="font-poppins font-medium text-2xl md:text-4xl text-primary leading-tight">
                  {orderData.trackingInfo 
                    ? `Status: ${orderData.trackingInfo.status.replace(/_/g, ' ')}` 
                    : `Order ${orderData.orderNumber}`}
                </h2>
                
                {orderData.trackingInfo && (
                  <p className="font-poppins text-sm text-stone-500">
                    Last updated location: {orderData.trackingInfo.location}
                  </p>
                )}
              ...
              <div className="flex flex-col w-full lg:w-[420px] gap-[30px] pt-[10px]">
                  ...
                  {orderData.items?.map((item: any) => (
                    <div key={item.id} className="flex flex-row items-center w-full p-4 bg-white border border-stone-200 rounded-lg shadow-sm gap-4 relative">
                      ...
                      <div className="flex flex-col flex-1 justify-center gap-1">
                        <h4 className="font-poppins font-medium text-sm text-primary leading-tight">
                          {item.variant?.product?.name || 'Unknown Item'}
                        </h4>
                        <div className="flex flex-row gap-2 items-center">
                          <span className="font-poppins font-light text-xs text-stone-500 uppercase tracking-wider">
                            Qty: {item.quantity}
                          </span>
                        </div>
                      </div>
                ...
                <div className="w-full pt-6 border-t border-stone-200 flex justify-between items-center">
                  <span className="font-poppins font-medium text-sm text-stone-500">Order Total</span>
                  <span className="font-poppins font-semibold text-lg text-primary">Rs. {orderData.total?.toFixed(2)}</span>
                </div>
